...
// Chave de API compartilhada com o backend (mesma env `API_KEY` nos dois projetos).
// O backend exige o header `x-api-key`; só o frontend, que porta o segredo, alcança a API.
// Ausente em dev local (o guard do backend libera tudo quando `API_KEY` não está definida).
const API_KEY = process.env.API_KEY;

/** Header de autenticação da API (`x-api-key`), quando a chave está configurada. */
...
export async function apiRequestBinary(
  ...
  let response: Response;
  try {
    response = await fetch(`${API_URL}${path}`, { headers: apiKeyHeaders(), cache: 'no-store' });
  } catch {
    return { ok: false, status: 0, message: 'Não foi possível conectar à API.' };
  }
